...
const { getSnapshotsByRange } = windHistory;
...
router.get("/", async (req, res) => {
  try {
    ...
    if (from && to) {
      ...
      const snapshots = getSnapshotsByRange(fromMs, toMs);
    ...
    if (period) {
      ...
      if (!validPeriods.includes(period)) {
        return res
          .status(400)
          .json({ error: `Invalid period. Valid: ${validPeriods.join(", ")}` });
      }

      const snapshots = windHistory.getSnapshots(period);
      const marine = await getMarineData().catch(() => null);

      // Fall back to live data if no history available yet
      if (snapshots.length === 0) {
        const wind = await getWindData();
        if (!wind) {
